...
void ControlRelay_BysoilMinMax() {
  ...
  for (int k = 0; k < 4; k++) {
    ...
    Serial.print("init_checkSoil : ");
    Serial.println(init_checkSoil[k]);
  }
  if (soil_error == 1) {
    for (int c = 0; c < 4; c++) {
      if (Max_Soil[c] > 0 ) {
...
void setAll_config() {
  for (int b = 0; b < 4; b++) {
    ...
    Serial.print(b);
    Serial.print(" : ");
    Serial.println(Max_Soil[b]);
    Serial.print("Min_Soil   ");
    Serial.print(b);
    Serial.print(" : ");
    Serial.println(Min_Soil[b]);
    Serial.print("Max_Temp   ");
    Serial.print(b);
    Serial.print(" : ");
    Serial.println(Max_Temp[b]);
    Serial.print("Min_Temp   ");
    Serial.print(b);
    Serial.print(" : ");
    Serial.println(Min_Temp[b]);
  }
  int count_in = 0;
  for (int eeprom_relay = 0; eeprom_relay < 4; eeprom_relay++) {
    for (int eeprom_timer = 0; eeprom_timer < 3; eeprom_timer++) {
      for (int dayinweek = 0; dayinweek < 7; dayinweek++) {
        ...
        Serial.print("time_open  : ");
        Serial.println(time_open[eeprom_relay][dayinweek][eeprom_timer]);
        Serial.print("time_close : ");
        Serial.println(time_close[eeprom_relay][dayinweek][eeprom_timer]);
        count_in++;
      }
    }
  }
